...
public class GestionCoursController implements Initializable {
    ...
    @FXML
    private ComboBox<String> combo_prof;
    ...
    private final ObservableList<String> profs = FXCollections.observableArrayList();
    private final ObservableList<String> questions = FXCollections.observableArrayList();
    private List<Cours> Cours = new ArrayList<>();
    private List<Prof> listeProfs = new ArrayList<>();
	...
	{
		Prof p = sc.rechercheProf(combo_prof.getValue());
		
		sc.ajouterCour(new Cours(libelle_cour.getText(), nom_cour.getText(),pdf_cour.getText(), p));
		listCours();
	}
	@FXML
	...
	private void listCours() throws SQLException
	{
		...
		for (Prof prof : listeProfs) {
			
			profs.add(prof.getNomProf());
		}
